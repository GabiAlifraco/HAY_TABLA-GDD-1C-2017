...
namespace UberFrba.Registro_Viajes
{
    public partial class VerViajes : Form
    {
        ...
        private void mostrarConsulta()
        {
            ...
            {
                string query = String.Format("SELECT TOP 5 CONCAT(Chofer.Cho_Nombre,' ', Chofer.Cho_Apellido) as Chofer, CONCAT(Cliente.Cli_Nombre,' ', Cliente.Cli_Apellido) as Cliente, V.Vi_AutoPatente AutoUtilizado, Turno.Turno_Descripcion, CONCAT(DAY(V.Vi_Inicio),'/', MONTH(V.Vi_Inicio),'/',YEAR(V.Vi_Inicio)) as Fecha, CONCAT(DATEPART(HH,V.Vi_Inicio),'hs ',DATEPART(mi,V.Vi_Inicio), 'min') as HoraInicio, CONCAT(DATEPART(HH,V.Vi_Fin),'hs ',DATEPART(mi,V.Vi_Fin), 'min') as HoraFin, V.Vi_ImporteTotal Importe FROM HAY_TABLA.Viaje V INNER JOIN Hay_TABLA.Chofer ON Chofer.Cho_Id = V.Vi_IdChofer INNER JOIN Hay_TABLA.Cliente ON Cliente.Cli_Id = V.Vi_IdCliente INNER JOIN Hay_TABLA.Turno ON Turno.Turno_Id = V.Vi_IdTurno WHERE Cli_Id = 1 AND (Vi_Inicio BETWEEN " + trimestre + ") ORDER BY Vi_Inicio DESC");
